 ...
 * Set pins according to message from host.
 * Return state of pins to host.
 * ========================================
*/
#include <device.h>
#include "tracer.h"
...
void main()
{
	...
	for(;;){
		if(USBFS_1_bGetEPState(1) == USBFS_1_OUT_BUFFER_FULL) {
			...
			INbuffer[0] = OUTbuffer[0]; // this is message number
...
int processHostOutput(uint8* OUTbuffer, uint16* mypins) 
{
    uint8 messagenumber, pinnumber;
	
	messagenumber = OUTbuffer[0];
	
	for(pinnumber = 0; pinnumber < NUMBERPINS; pinnumber++) {
		if(OUTbuffer[pinnumber + 1]) {
